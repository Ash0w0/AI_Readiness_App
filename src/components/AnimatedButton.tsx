...
export function AnimatedButton({
  ...
  const variantClasses = {
    ...
    journey: 'journey-button text-white font-semibold'
  };
  ...
  return (
    <motion.button
      ...
      whileHover={!disabled && !loading ? { 
        scale: variant === 'journey' ? 1.01 : 1.01,
        y: variant === 'journey' ? -1 : 0
      } : {}}
      whileTap={!disabled && !loading ? { 
        scale: variant === 'journey' ? 0.99 : 0.99,
        y: variant === 'journey' ? 0 : 0
      } : {}}
      transition={{ duration: 0.2 }}
    >
      {/* Regular shimmer effect for non-journey buttons */}
      {variant !== 'journey' && (
        <motion.div
